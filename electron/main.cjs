...
const selfTest = process.argv.includes('--self-test') || browserTest || regressionTest;
const arg = (name) => {
...
const testRoot =
  selfTest && app.isPackaged
    ...
    : root;
if (selfTest) {
  require('node:fs').mkdirSync(path.join(testRoot, 'qa', 'test-profile'), { recursive: true });
...
function registerIpcHandlers() {
  ...
  handle('atlas:export', async ({ bytes, name }) => {
    ...
    const safeName =
      typeof name === 'string' ? name.replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 80) : 'atlas-map';
      ...
      submenu: [
        {
          ...
          click: () =>
            win.webContents.executeJavaScript("document.getElementById('open-folder').click()"),
        },
        { role: 'close' },
      ],
    },
...
app
  ...
  .then(async () => {
    ...
    const fixture = regressionTest
      ? await require('./regression-test.cjs').createFixture(path.join(testRoot, 'qa'))
      : null;
    ...
    if (selfTest) {
      try {
        if (regressionTest) {
          await require('./regression-test.cjs').run(win, testRoot, fixture, browserTest);
        } else if (browserTest) await require('./browser-test.cjs')(win, testRoot, requested);
