...
bool WaterWriter::BuildAndWrite(std::string zone_name) {
...
bool WaterWriter::BuildAndWriteS3D(std::string zone_name) {
	...
	for(uint32_t i = 0; i < zone_frags.size(); ++i) {
		if(zone_frags[i].type == 0x21) {
			...
			WaterWriterWriterRegionType region_type = WriterRegionTypeUntagged;
...
bool WaterWriter::BuildAndWriteEQG(std::string zone_name) {
		...
		for (uint32_t i = 0; i < region_count; ++i) {
			...
			if (region->GetName().length() >= 3) {
				...
				if(region_code.compare("AWT") == 0) {
					...
					region_type = WriterRegionTypeGeneralArea;
				}
				else {
...
bool WaterWriter::BuildAndWriteEQG4(std::string zone_name) {
	eqLogMessage(LogTrace, "Loading standard eqg %s.eqg", zone_name.c_str());
