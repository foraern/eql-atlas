...
// Conservative AABB test in navigation coordinates; exact queries still classify affected voxels.
bool WaterMapV2::PotentiallyBlocked(const glm::vec3& low,const glm::vec3& high) const {
 for(auto& region:regions){if(!BlocksWalking(region.first))continue;auto& box=region.second;
 glm::vec3 lo(INFINITY),hi(-INFINITY);
 for(float x:{box.GetMinX(),box.GetMaxX()})for(float y:{box.GetMinY(),box.GetMaxY()})for(float z:{box.GetMinZ(),box.GetMaxZ()}){
 auto v=box.GetTransformation()*glm::vec4(x,y,z,1);glm::vec3 p(v.y,v.z,v.x);lo=glm::min(lo,p);hi=glm::max(hi,p);}
 if(lo.x<=high.x&&hi.x>=low.x&&lo.y<=high.y&&hi.y>=low.y&&lo.z<=high.z&&hi.z>=low.z)return true;
 }return false;
}
